...
| `USER_AI_GLOBAL_DAILY` | 선택 | 로그인 사용자(체험 계정 제외) 전체의 Claude 호출(사진 인식·AI 레시피·가져오기·식단 초안·사 먹으면 얼마·영양 추정) 24시간 예산(코드 기본 400, `render.yaml` 100). 넘으면 429 `조금 뒤에 다시 써주세요`, 영양 추정은 AI 없이 계산. 체험 계정 호출은 세지 않는다 | 직접 설정 |
| `USER_NUTRITION_GLOBAL_DAILY` | 선택 | 로그인 사용자 전체의 영양 추정(AI) 하루(서울 날짜) 횟수(코드 기본 60, `render.yaml` 40). 위 예산 안에서 따로 한 번 더 센다. 넘으면 AI 추정 없이 계산 | 직접 설정 |
...
5. 기본 채널이 확정되면 `backend/app/data/default_channels.json`에 `[{"channel_id": "UC…", "name": "메모용 이름"}]` 모양으로 적고 배포한 뒤 Render Shell에서 `flask seed-default-channels`(목록에서 뺀 채널은 기본 채널에서 꺼진다). **지금 파일에는 사용자가 확정한 14개가 들어 있다(`15df7e0`).**
...
2. 체험 계정은 `provider=demo`, 닉네임 `체험 사용자`로 만들어지고 예시 재고 10개(두부 D-1·대파 D-2 포함)·필수품 73개(모든 계정에 들어가는 기본 필수품 72개 + 간장 — 간장만 재고에 없어 첫 화면 `떨어졌어요` 배너에 뜬다)·레시피 2개·양념 비율 1개가 들어 있다.
...
5. **IP 한도 확인(배포 후 한 번):** IP 한도는 `X-Forwarded-For`의 뒤에서 `TRUSTED_PROXY_HOPS`(기본 1)번째 값을 접속 주소로 본다.
   - **꾸민 헤더가 통하지 않는지:** 같은 컴퓨터에서 앞쪽 값만 바꿔 `DEMO_IP_HOURLY_LIMIT`+1번(기본 31번) 보낸다. 마지막이 `429`면 정상이다(꾸민 값이 무시되고 같은 주소로 셈). 모두 `200`이면 앞쪽 값을 믿고 있는 것이니 `TRUSTED_PROXY_HOPS`를 줄인다. 확인 뒤 만든 체험 계정은 24시간 뒤 지워진다.
     ...
     ```
   - **다른 사람이 막히지 않는지:** 위 확인 직후(한 시간 안) 다른 네트워크의 기기(와이파이를 끈 폰 LTE 등)에서 `로그인 없이 체험하기`를 누른다. 들어가지면 정상이다. `지금 같은 인터넷으로 체험하는 분이 많아요`가 나오면 모든 사람이 프록시 주소 하나로 묶인 것이니 `TRUSTED_PROXY_HOPS`를 늘려 다시 배포하고 두 확인을 반복한다.
