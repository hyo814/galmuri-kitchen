...
- Produces:
  ...
  - **구현 때 바뀐 것(2026-09-14):** 401(`signOut`)은 기기 데이터를 지우지 않고 `pauseShopping()`으로 보내기만 멈춘다(대기 변경·목록·사진은 주인과 함께 남아 같은 사람이 다시 로그인하면 이어서 보낸다, 다른 사람이면 `rememberUser`가 지운다). `clearShoppingDevice()`는 더보기에서 직접 로그아웃할 때(`logout`)만 부른다.
