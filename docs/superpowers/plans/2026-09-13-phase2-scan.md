...
**Architecture:** 백엔드는 `ai.py`(Anthropic SDK 호출·프롬프트·예시 결과)와 `scan.py`(업로드 검증·일일 한도·결과 정리)로 나눈다. 테스트는 네트워크를 쓰지 않고 `app.ai.extract`와 `anthropic.Anthropic`을 가짜로 바꾼다. 호출 기록은 새 `ai_calls` 테이블에 성공한 호출만 남긴다. 확인 화면은 `POST /api/ingredients/bulk`로 여러 재료를 한 트랜잭션에 넣는다(하나라도 틀리면 전부 취소). 프론트는 브라우저에서 사진을 긴 변 1568px JPEG로 줄여 올리고, 시트 하나(`ScanSheet`) 안에서 고르기 → 찾는 중 → 확인(`ScanReview`) → 실패 화면을 바꿔 보여 준다.
