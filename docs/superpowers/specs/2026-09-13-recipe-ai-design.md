...
- **영상 설명·캡션 미리 거르기 (2026-09-16, 사용자 승인):** 유튜브 설명(제목은 보지 않는다 — `계란 2개로…` 같은 제목은 레시피가 아니다)이나 인스타그램 캡션에 블로그와 같은 재료·양 표시(`RECIPE_SIGNAL`)가 하나도 없으면 AI를 부르지 않고 422 `need_text`(`link` 기록·토큰 없음, 외부 요청 `link_fetch` 기록은 그대로). 문구는 유튜브 `영상 설명에서 레시피를 찾지 못했어요. 설명에 있으면 복사하고, 쇼츠처럼 영상에만 있으면 보면서 재료와 만드는 법을 적어 아래에 붙여 넣어주세요.`, 인스타그램 `게시물 설명에서 레시피를 찾지 못했어요. 설명에 있으면 길게 눌러 복사하고, 릴스처럼 영상에만 있으면 보면서 재료와 만드는 법을 적어 아래에 붙여 넣어주세요.`(표시 규칙이 놓친 표기 `간장 3T`·`2숟가락`이거나 미리보기 캡션이 잘렸을 수 있어 복사도 안내한다). 설명이 비었거나 10자 미만이어도 이 문구다. 인스타그램 미리보기가 한국어(`좋아요 1,234개`)로 오면 그 수가 표시로 잡혀 거르지 못한다(예전처럼 AI를 부른다). 계기: 설명에 재료가 없는 쇼츠 링크(`m.youtube.com/shorts/…`)가 AI를 불러 하루 횟수만 쓰고 `need_text`로 끝났다(자막은 가져오지 않는다). 블로그는 본문 사진을 함께 읽을 수 있어 그대로, 붙여 넣은 글도 거르지 않는다.
