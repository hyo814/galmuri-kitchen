...
- **주 보기(`Meals.tsx` `PlanWeek`):** 몸 정보(`useResource("/api/body-profile")`)로 `profileTarget`을 구해 `goal = goalFor(profileTarget, plan.goal_kcal)`. 하루 카드마다 `daySum`이 있으면 머리 전체가 `button.nt-head-btn`(하루 영양 시트를 여는 `aria-haspopup="dialog"`)이 되고 오른쪽에 `dayHeadText`(목표 초과 시 `.nt-kcal.warn` 주황), 머리 아래 목표 막대(`.nt-dmeter`, 넘으면 `.warn`); 없으면 지금처럼 `{filled} / 4`. 칸 줄 오른쪽에 `.nt-slot-kcal`(`slotKcalText`), `aria-label` 끝에 `, 1인분 {kcal}`. `BodyGoalCard`에 `todayTotal`(오늘이 식단 기간 안이고 오늘 칸 합계가 있을 때만) — 카드 둘째 줄 `오늘 식단` 막대(`.nt-meter.grow`)·수치(`.nt-num`). **채우기:** 식단을 받은 뒤 `user.nutrition !== "off"`이고 보이는 주의 pending 레시피(`fillTargets`)가 있으면 식단·레시피 조합마다 한 번만(모듈 `Set filledOnce`, `resetMealsView`가 비운다) `POST /api/nutrition/fill` → 끝나면(실패해도 조용히) 식단을 다시 받는다.
