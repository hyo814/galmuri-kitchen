...
| GET/PUT/DELETE | `/api/recipes/<id>` | 상세 / 수정 / 삭제. 상세의 `ingredients`는 `[{name, amount, have, matched_name}]`(현재 재고 기준). GET 상세에는 `cooked`(요리 표시)도 붙는다(29절 결정 26) |
...
- **일기 상세·고치기·지우기 `GET/PATCH/DELETE /api/cook-logs/<id>`(29절 결정 17·18, Task 5):** GET은 `detail_json`(`Cache-Control: no-store`, 남의 것·범위 밖 id 404). PATCH는 `{cooked_on?, rating?, memo?, eat_out_price?}`만 받는다 — body가 dict가 아니거나 그 밖 키(`servings`·`usages`)가 있으면 400(인분·쓴 재료는 재고와 어긋나서 못 고친다는 안내는 화면이 따로 한다). 저장과 같은 `parse_common(data, log, creating=False)`을 쓰고, `eat_out_price`를 보냈으면 `summarize(log.eat_out_price, log.servings, log.items)`로 재료비·아낀 돈·제외 수를 다시 계산한다. 날짜를 바꿔도 연결된 먹은 기록 날짜는 그대로 둔다. DELETE는 사진 키를 모아 일기만 지우고(재고·먹은 기록은 그대로, 결정 17) 커밋 뒤 `storage.delete`.
- **일기 사진 바꾸기·지우기 `PUT/DELETE /api/cook-logs/<id>/photo`(결정 19, Task 5):** PUT 순서 — 저장소 off 503 → `get_owned_or_404` → `photos.read_image(MAX_PHOTO_BYTES)` → `lock_user` → `check_photo_room(g.user, size, excluding=log)`(자기 일기의 옛 사진은 합계에서 뺀다) → 새 키로 `storage.put` → 옛 키를 기억해 두고 칸을 바꾼 뒤 커밋(실패하면 방금 올린 새 파일을 지운다) → 옛 파일 삭제 → 200 `detail_json`. DELETE는 사진이 없어도 204, 커밋 뒤 파일을 지운다.
- **레시피 상세 요리 표시 `recipe_cooked(recipe_id, user_id)`(29절 결정 26, Task 5):** 그 사용자의 일기 중 `recipe_id`가 같은 것이 없으면 `null`, 있으면 `{count, last_on, last_rating}`(마지막은 `cooked_on`·id가 가장 큰 일기). `GET /api/recipes/<id>`에서 `from .cooklog import recipe_cooked`(cooklog가 recipes를 import하므로 함수 안에서만)로 불러 붙이고, 레시피 목록·생성·수정·삭제 응답(`recipe_json`)은 바꾸지 않는다.
