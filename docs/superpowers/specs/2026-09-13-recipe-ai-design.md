...
- 화면: (2026-09-15, 4b-3 Task 8) 달력 칸을 누르면 `FoodLogDaySheet`가 열린다(`FoodLog.tsx`의 `open`·`selected` state, 시트를 닫아도 `selected`는 남겨 칸에 `.sel`을 유지). 시트 제목은 `slotDateText(date, "")`(오늘도 `· 오늘` 없이, 시안), 설명은 `dayDescription`(결정 15: kcal 있는 기록만 합산, 목표는 몸 정보 하루 칼로리 목표만 — 식단 `goal_kcal`은 쓰지 않는다). 목표·합계가 있으면 설명 아래 `.nt-meter.nt-dmeter`(넘으면 `.warn`, 아침·점심·저녁 주 보기와 같은 계산), 나트륨이 2,000mg을 넘으면 `.fl-warn` 문구. 끼니(`MEALS`)마다 `.fl-meal` 묶음: 기록은 `button.fl-log`(사진·이름·집밥·외식 배지·이름 없음 배지·양·kcal·별점·메모, 눌러 고치기 — Task 9가 연결), 그날 아직 기록과 안 이어진 식단 칸은 `.fl-plan`(결정 14, `withJosa`로 조사 붙임) 한 줄과 `먹었어요` 버튼(`POST /api/meal-slots/<id>/eaten` → `forgetResources`로 식단·먹은 기록 캐시를 비우고 다시 받음), 끝에 `.fl-addrow`(`+ 먹은 것`, 오늘 뒤 날짜는 방어적으로 숨김). **채우기 흐름(개정 1 S10):** 같은 파일의 모듈 `Set fillAttempted`(레시피 id만 담음, `resetFoodLogView`가 비운다)로 `nutrition_pending_recipe_ids` 중 아직 시도 안 한 것만 요청 **전에** 표시하고 `POST /api/nutrition/fill` 한 번 → 끝나면 조용히 실패해도 날짜를 다시 받는다 — 4b-2 `Meals.tsx attempted`·`RecipeNutrition filled`와 달리 재시도 횟수를 세지 않고 레시피 id 하나당 이 세션에서 딱 한 번만 부른다(날짜·조합 키로 기억하지 않아 pending이 줄어도 새로 부르지 않음). `FoodLog.tsx`의 `onChanged`는 달 `reload`(자식이 올려준 콜백)와 `forgetResources("/api/food-logs?date=")`를 부른다.
