...
- 체험하기 계정(추가: 2026-09-14, 사용자 결정 "체험 계정 + 카카오 로그인"): `DEMO_LOGIN=1`일 때만 `POST /api/demo-login`이 열린다. 누를 때마다 새 계정(`provider=demo`)과 예시 데이터를 만들고 24시간 뒤 `flask purge-demo-users`로 지운다. IP(IPv6는 /64)별 1시간 3개·하루 10개, 전체 5,000개(차면 오래된 체험 계정부터 재활용), IP는 `SECRET_KEY`에서 뽑은 키의 HMAC 해시만 남긴다. 체험 계정 AI 하루 한도 5번(2026-09-15 3→5, 사용자 결정), 체험 전체 AI 24시간 예산(`DEMO_AI_GLOBAL_DAILY`)을 넘으면 예시 결과, 영상은 받아 둔 기본 채널 영상을 읽기만 한다(새로 받기·채널 바꾸기 없음 — 보기·검색은 유튜브 할당량을 쓰지 않는다. 영상 화면 `레시피로 가져오기`만 링크 가져오기와 같은 길이라 AI 레시피 한도 검사를 먼저 통과해야 `videos.list` 1 unit을 쓴다: 체험 계정 하루 5번, 체험 전체 AI 80번이면 예시 결과로 바뀌어 유튜브를 부르지 않는다. 쓸 수 있는 영상이 없으면 예시 목록, 2026-09-15 사용자 결정). 세션은 `user_id`와 `provider_id`가 함께 맞아야 한다. `ai_calls`는 사용자를 지워도 남는다(`SET NULL`, `demo` 표시). 예시 데이터에 먹은 기록 3개(24절 결정 16), 요리 일기 예시 3개·버린 재료 2개·예시 재료 가격·예시 레시피 사 먹으면 얼마(체험은 요리했어요 시트에서 자동 추정하지 않음 — 값 없는 레시피는 추정해줘요를 눌러야 AI, 29절 결정 30)도 함께 만든다.
...
- **영상 설명·캡션 미리 거르기 (2026-09-16, 사용자 승인):** 유튜브 설명(제목은 보지 않는다 — `계란 2개로…` 같은 제목은 레시피가 아니다)이나 인스타그램 캡션에 블로그와 같은 재료·양 표시(`RECIPE_SIGNAL`)가 하나도 없으면 AI를 부르지 않고 422 `need_text`(`link` 기록·토큰 없음, 외부 요청 `link_fetch` 기록은 그대로). 표시는 이날 `숟가락`·`숟갈`과 숫자 뒤 `공기`·`T`·`t`·`ts`·`tbsp`·`tsp`를 더해 넓혔다(`간장 3T 설탕 1T 참기름 1t`·`간장 2숟가락, 밥 1공기`만 적은 설명도 AI가 읽는다. 뒤에 영문이 붙으면 양이 아니다 — `5ton`·`5GB`). 문구는 유튜브 `영상 설명에서 레시피를 찾지 못했어요. 설명에 있으면 복사해 붙여 넣고, 영상에만 있으면 보면서 재료와 만드는 법을 아래에 적어주세요.`, 인스타그램 `게시물 설명에서 레시피를 찾지 못했어요. 설명에 있으면 길게 눌러 복사해 붙여 넣고, 영상에만 있으면 보면서 재료와 만드는 법을 아래에 적어주세요.`(레시피가 없다고 단정하지 않는다 — 표시 규칙이 놓친 표기 `양배추 1/4통`이거나 미리보기 캡션이 잘렸을 수 있다). 설명이 비었거나 10자 미만이어도 이 문구다. 인스타그램 미리보기가 한국어(`좋아요 1,234개`)로 오면 그 수가 표시로 잡혀 거르지 못한다(예전처럼 AI를 부른다). 계기: 설명에 재료가 없는 쇼츠 링크(`m.youtube.com/shorts/…`)가 AI를 불러 하루 횟수만 쓰고 `need_text`로 끝났다(자막은 가져오지 않는다). 블로그는 본문 사진을 함께 읽을 수 있어 그대로, 붙여 넣은 글도 거르지 않는다.
...
- **장보기 미리보기 (2026-09-14, 4b-1 Task 4 — 결정 2·6을 따른다, `app/amounts.py`·`meals.py`):**
  ...
  - `GET /api/meal-plans/<id>/shopping-preview` → `{name, start_on, end_on, recipe_slot_count, buy, manual, skip}`(`name`은 담기 `source_label`용 식단 이름). `start_on = max(seoul_today(), plan.start_on)`(결정 2: 오늘 이후 끼니만), `end_on`은 식단 종료일. `start_on` 이후(포함)이고 레시피가 있는 칸만 `recipe_slot_count`에 세고, 칸마다 `ratio = slot.servings / max(recipe.servings, 1)`(방어)로 `needs`를 만든다. 재고·장보기 목록은 이 사용자 것만. 기간이 모두 지나 셀 칸이 없으면 `recipe_slot_count 0`·빈 묶음(오류 아님). 담기는 이 API가 하지 않는다 — 화면이 `POST /api/shopping/items/bulk` `{source: "meal_plan", source_label, items:[{name, quantity, unit, planned_on}]}`로 담는다(목록에 있는 이름은 서버가 또 건너뛴다).
...
- **하루 영양 시트(`DayNutritionSheet.tsx`, `{date, slots, goal, onClose}`):** 그날 채운 칸을 `daySum`으로 더해 kcal/목표 막대, `source calc`인 칸이 하나라도 있으면 탄단지 비율 막대·범례(`.nt-split`·`.nt-legend`)와 당류·나트륨 상자(`sugarDay`·`sodiumDay`, 초과 시 주황), AI 추정 칸이 섞이면(`hasAi`) 상자 아래(또는 계산 칸이 없으면 단독으로) `kcal 일부는 AI 추정치예요`(결정 16). 빠진 영양소는 `이상`·안내로(결정 14 개정 2, 아래 2026-09-16 구현 세부). 칸마다 `아침 · 토스트  310kcal`(kcal 없으면 `—`), 끝에 참고용 문구.
...
- **레시피 상세 영양 카드·하루 영양 시트:** 빠진 영양소는 `탄수화물 30g 이상`·`나트륨 7mg 이상`·`나트륨 알 수 없어요`(` 이상`까지 값 글자(굵게, 본문 색) 안에), 탄단지 중 하나라도 빠지면 비율 막대(`.nt-split`)를 숨기고, 당류·나트륨 기준치 글자·막대는 위 규칙대로. 안내는 `.nt-note`(13px, `--text-2`)로 카드는 당류·나트륨 아래, 하루 시트는 당류·나트륨 상자 아래. 하루 시트는 `source calc` 칸의 이름만 모은다(AI 추정 칸은 원래대로 `kcal 일부는 AI 추정치예요`).
...
- **체험 계정 요리 일기 예시(29절 결정 30, Task 7, `app/demo.py`):** 예시 재료 중 두부·대파·김치·돼지고기 앞다리살에 `INGREDIENT_PRICES`로 가격·구입 수량(만들 때 수량 그대로)을 미리 넣는다. 체험 레시피 둘(김치찌개·된장찌개) 모두 `EAT_OUT_PRICES`로 사 먹으면 얼마를 미리 넣는다 — 체험은 요리했어요 시트를 열어도 자동으로 추정하지 않으므로(개정 1 P16) 값 없는 레시피가 남지 않게 한다(김치찌개는 `sample` = AI 예시 모드와 같은 9,000원, 된장찌개는 `user` = 직접 넣은 모양). 요리 일기 3개(`COOK_LOGS`, 오늘로부터 2·5·35일 전)를 `cooklog.item_cost`·`cooklog.summarize`·`cooklog.is_seasoning`으로 계산해 재료비·아낀 돈·가격 제외 수까지 채우고, 버린 재료 2개(`DISCARDED`, `ingredient_removals(reason="discarded")`)도 만든다. 재고 수량은 요리 일기 때문에 줄지 않는다(예시 재고는 그대로 둔다).
