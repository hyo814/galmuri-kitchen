...
- **장보기 목록 만들기 화면 (2026-09-15, 4b-1 Task 9, 시안 ShoppingPreview·ShoppingPreviewBottom):**
  ...
  - 네 묶음(빈 묶음은 숨김): `모자란 만큼 담아요`(체크 기본 켬, 오른쪽 `1모 담기`) · `단위가 달라요 · 직접 골라주세요`(체크 기본 끔, 오른쪽 `2판`) · `양념 · 집에 없으면 골라주세요`(체크 기본 끔, 오른쪽 `1개`, 설명 `2큰술 필요 · 없어요` — 집에 있지만 재고에 안 넣은 양념이 많다) · `담지 않아요`(체크 없음, `목록에 있어요`/`충분해요`). 버튼 `N개 장보기에 담기`는 체크한 줄만 센다. 줄 설명은 `plan.ts previewDetail`(필요·있음 양은 딱 떨어지는 분수 `½`·`2½`가 아니면 소수 첫째 자리까지, `.0`은 빼고 아주 적어도 `0.1` — `1.7개 필요 · 1개 있어요`, `616.7g 필요 · 600g 있어요`), 살 날 태그는 `buyDayText`(오늘·지남 `오늘 사요`, 아니면 `16일(수)에 사요`) + `식단` 태그. 체크는 사용자가 바꾼 것만 기억해 캐시로 먼저 보인 미리보기가 새로 와도 어긋나지 않는다.
...
- **계획하며 정한 것 (스펙·시안에 없던 빈틈, 사용자 확인 대상):**
  ...
  6. 숟가락 단위(`큰술`·`작은술`·`컵`·`꼬집` 등)나 `약간`처럼 양을 셀 수 없는 재료는 재고에 같은 이름이 있으면 `충분해요`(간장 2큰술 때문에 간장 한 병을 또 담지 않게), 이름이 맞는 재고가 전혀 없으면 따로 `양념 · 집에 없으면 골라주세요` 묶음(체크 없음, 담으면 한 통 `1개`)에 둔다(2026-09-16 사용자 결정 — 처음에는 `단위가 달라요`에 섞여 AI 초안 레시피 11개를 넣은 운영 식단에서 양념 11줄이 그 묶음을 채웠다. 체크해 두면 집에 있지만 재고에 안 넣은 양념까지 담긴다). `단위가 달라요`는 필요한 단위가 둘 이상이거나 재고가 다른 단위일 때만 쓴다.
...
- **식단 → 장보기 수량 비교 (D4, 4b단계):** 필요량(인분 배율 적용 합계)과 재고를 이름 매칭 후 단위가 같으면 `필요 − 보유`가 0보다 클 때만 그만큼 담는다.
  단위가 다르거나 파싱 불가면 미리보기에 `있음 2개 · 필요 12개`처럼 보여 주고 사용자가 체크해 담는다. 숟가락·셀 수 없는 양뿐인 재료는 이름이 맞는 재고가 있으면 담지 않고, 전혀 없으면 `양념` 묶음에 체크 없이 둔다(담으면 한 통 1개). 담을 양은 정수로 올린다(2026-09-16 사용자 결정). 20절의 "없는 것만"은 이 방식으로 대체한다. (구현: `app/amounts.py`·`meals.shopping_rows`)
