...
- 체험하기 계정(추가: 2026-09-14, 사용자 결정 "체험 계정 + 카카오 로그인"): `DEMO_LOGIN=1`일 때만 `POST /api/demo-login`이 열린다. 누를 때마다 새 계정(`provider=demo`)과 예시 데이터를 만들고 24시간 뒤 `flask purge-demo-users`로 지운다. IP(IPv6는 /64)별 1시간 3개·하루 10개, 전체 5,000개(차면 오래된 체험 계정부터 재활용), IP는 `SECRET_KEY`에서 뽑은 키의 HMAC 해시만 남긴다. 체험 계정 AI 하루 한도 5번(2026-09-15 3→5, 사용자 결정), 체험 전체 AI 24시간 예산(`DEMO_AI_GLOBAL_DAILY`)을 넘으면 예시 결과, 영상은 받아 둔 기본 채널 영상을 읽기만 한다(새로 받기·채널 바꾸기 없음 — 보기·검색은 유튜브 할당량을 쓰지 않는다. 영상 화면 `레시피로 가져오기`만 링크 가져오기와 같은 길이라 AI 레시피 한도 검사를 먼저 통과해야 `videos.list` 1 unit을 쓴다: 체험 계정 하루 5번, 체험 전체 AI 80번이면 예시 결과로 바뀌어 유튜브를 부르지 않는다. 쓸 수 있는 영상이 없으면 예시 목록, 2026-09-15 사용자 결정). 세션은 `user_id`와 `provider_id`가 함께 맞아야 한다. `ai_calls`는 사용자를 지워도 남는다(`SET NULL`, `demo` 표시). 예시 데이터에 먹은 기록 3개(24절 결정 16), 요리 일기 예시 3개·버린 재료 2개·예시 재료 가격·예시 레시피 사 먹으면 얼마(체험은 요리했어요 시트에서 자동 추정하지 않음 — 값 없는 레시피는 추정해줘요를 눌러야 AI, 29절 결정 30)·예시 식단 1개(오늘부터 7일·2인분·11칸, 2026-09-16 사용자 승인 — 저녁은 매일·점심은 사흘·아침은 하루만 채워 주 보기에 빈 날이 없고 AI 초안이 채울 빈 칸이 남는다. 오늘 저녁은 레시피 칸(된장찌개)이고 오늘 아침은 먹은 기록 토스트와 겹치지 않게 비운다. 예시 레시피 둘은 같은 날·이어진 날에 두지 않는다. 직접 쓴 칸 6개(토스트·카레라이스·김밥·비빔밥·잔치국수·제육덮밥)는 식약처 식품영양성분 자료집(2020) 음식 1인분 kcal을 `est_kcal`로 넣어 AI 초안 칸처럼 kcal이 보인다 — 20절 계획 8의 체험 예외)도 함께 만든다.
