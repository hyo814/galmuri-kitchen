...
- 화면: (2026-09-15, 4b-3 Task 10, 시안 3 · ADD·4 · PHOTO FIRST) `FoodLogSheet.tsx`가 `MAX_LOG_PHOTOS = 4`와 `uploadFoodPhoto<T>(url, file)`(`resizeImage`로 긴 변 1568px JPEG로 줄여 `FormData` `image` 필드로 `POST`)를 내보낸다. **시트 사진 칸**(만족도 아래, 메모 위, `사진 (선택)`): `div.sh-photos`에 이미 올린 사진(`log.photos`, 고치기)을 `div.sh-photo`(`img alt="사진 {n}"` + `button.fl-photo-x`(`aria-label="사진 {n} 빼기"`, `Icon close`) — 누르면 바로 `DELETE /api/food-logs/<id>/photos/<photo_id>`, 확인 없이, 실패는 칸 아래 `p.error`)로 먼저 보여주고, 그다음 아직 안 올린 새 사진(`URL.createObjectURL` 미리보기, 같은 `fl-photo-x`로 로컬에서만 빼기 — 시트가 닫힐 때 `useEffect` 정리로 `revokeObjectURL`)을 보여준 뒤, 합계가 4장 미만이면 `button.sh-photo.add`(`Icon camera` `찍기`, `input capture="environment" hidden`) · `button.sh-photo.add`(`Icon file` `앨범`, `input multiple hidden`) — 앨범에서 남은 자리보다 많이 고르면 자리만큼만 넣고 `p.hint 사진은 4장까지 넣을 수 있어요`. **저장 순서(결정 9):** 기록 `POST`/`PATCH` 성공(바뀐 칸이 없으면 건너뜀) → `savedLog` state로 그 뒤부터 `log`(고치기)로 동작 → 새 사진을 `newPhotosRef`에서 하나씩 `uploadFoodPhoto`로 올림 → 모두 되면 새 사진 목록을 비우고 `onSaved`로 시트를 닫음. 하나라도 실패하면 성공한 것만 이미 올린 사진 목록으로 옮기고 남은 것(실패한 것 포함)은 새 사진 목록에 그대로 두고 `changing`을 꺼 고치기 모드로 보이며 `p.error role="alert" 사진 {N}장은 올리지 못했어요 · {서버 문구}` — 시트는 닫지 않는다(다시 `저장`하면 남은 것만 올림). **사진만 먼저(프레임 4):** `FoodLog.tsx` 머리 오른쪽 `button.icon-btn.fl-camera`(`aria-label="사진만 먼저 남기기"`, `aria-haspopup="dialog"`) → `Sheet` `빠르게 사진만 남기기`(설명에 끼니 시간대) → `div.actions.actions-even`의 `앨범에서 고르기`·`카메라로 찍기`(각 숨긴 `input`, 한 장) → 고르면 시트를 닫고 머리 아래 `p.muted role="status" 사진을 남기는 중…` → `uploadFoodPhoto<FoodLog>("/api/food-logs/photo", file)` → `forgetResources("/api/food-logs")` → 응답의 `eaten_on`으로 달을 옮기고(`monthOf`) 그 날짜 상세를 연다(`setSelected`·`setOpen(true)`). 실패는 머리 아래 `p.error role="alert"`(서버 문구 그대로, 503 포함). `styles.css`: `.fl-camera`(`margin-left: auto`로 머리 오른쪽 끝) · `.fl-photo-x`(28px 원, 터치 44px는 `::before { inset: -8px }`로). 브라우저 확인(5199→5181): 5장 앨범 선택 시 3장만 들어가고 안내 문구 확인, 이미 올린 사진 `DELETE` 즉시 반영, 네트워크를 끈 채 저장하면 시트가 남고 `p.error`가 뜨고 네트워크를 되돌려 다시 저장하면 남은 사진만 올라가 닫힘, PNG·WEBP 업로드, 텍스트 파일을 `.jpg`로 바꿔 올리면 415 문구, 달력 썸네일이 그날 첫 사진(끼니 순)과 일치, 다크 모드·키보드 포커스(`사진 {n} 빼기` 접근 이름) 확인.
