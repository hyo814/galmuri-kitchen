...
- **A. PC(넓이 1024px 이상), 모든 화면:** 왼쪽 소개(`index.html`의 `<aside class="pitch" aria-label="갈무리부엌 소개">`) — 로고 줄, `영수증 한 장 찍으면 / 오늘 저녁 메뉴까지 나와요`, 세 걸음, QR `폰으로 이어서 보기`. 오른쪽은 폰 넓이(420px) 앱을 둥근 틀에 둔다. 1024px 미만은 소개를 `display:none`으로 숨기고 화면은 그대로다.
  - 앱은 창 스크롤을 그대로 쓴다(스크롤 복원·무한 스크롤을 바꾸지 않게). 고정 틀 `body::before`가 둥근 구멍 밖을 책상 색(`--desk`)으로 덮는다. `--col-x`·`--col-w`·`--frame-top`·`--frame-h` 변수로 `.tabbar`·`.cta-bar`(fixed)와 `.sheet`(`<dialog>` top layer, 배경 `::backdrop` 포함)를 앱 칸 안에 맞춘다. 시작 화면(splash)만 전체 화면이다. 새 `position: fixed`를 만들면 이 묶음에도 넣는다.
...
- **B. 로그인 미리보기:** 로고와 버튼 사이에 3장(`찰칵 · 영수증 찍으면 끝이에요`, `잠깐만요 · 두부가 내일까지래요`, `그럼 오늘은 · 두부조림 어때요?`), 그림 없이 마크업으로 만든다. 4초마다 넘기고, 점을 누르면 그 장에서 멈춘다. 올려두거나 포커스가 안에 있으면 잠깐 멈추고, 움직임 줄이기면 넘기지 않는다. 캐러셀 `aria-roledescription`, 장마다 `n / 3`, 안 보이는 장은 `aria-hidden`+`inert`, 넘기는 동안 `aria-live="off"`·멈추면 `polite`, 점은 `n번째 미리보기` 버튼(`aria-current`). 화면 높이 700px 이하면 숨긴다(체험하기 버튼이 밀리지 않게). 버튼 순서·로직은 그대로.
- **C. 체험 계정 첫 30초(`provider = demo`):** 계정마다 한 번(`localStorage demoGuideDone:<user id>`, 닫기·두 버튼 중 하나를 누르면 기억).
  - 재고 맨 위 안내 카드: `냉장고를 미리 채워뒀어요. 그런데 **{가장 급한 재료}{이/가} {오늘까지예요|내일까지예요|N일 남았어요}!** {재료}{으로/로} 뭘 만들 수 있을지 같이 볼까요?`. 급한 재료 = 상태 `urgent`이고 아직 안 지난(D-0 이상) 재료 중 남은 날이 가장 적은 것. 없으면 `냉장고를 미리 채워뒀어요. 곧 먹어야 할 재료로 뭘 만들 수 있을지 같이 볼까요?`. 버튼 `요리 찾아보기`(레시피 추천 칸으로, 아래 짚기) · `사진 찍어보기`(재고의 사진으로 추가 시트, 스캔 off면 숨김) · ✕ `안내 닫기`.
  - 레시피 추천 칸 AI 카드 짚기: `요리 찾아보기`로 왔을 때 한 번(모듈 변수) AI 카드에 초록 테두리 + 말풍선 `여기를 눌러보세요. {A}랑 {B}부터 쓰는 요리 3가지를 바로 만들어줘요. 체험은 하루 {recipe_limit}번까지 돼요.`(하나면 `{A}부터`, 없으면 `곧 먹어야 할 재료부터`; 체험 한도는 `DEMO_AI_DAILY_LIMIT` 5). 카드 버튼에 포커스하고 말풍선은 `aria-describedby`. 카드를 누르거나 다른 곳을 누르거나 Esc·카드가 화면 밖으로 나가면 사라진다. AI 카드가 없는 모드면 짚지 않는다.
  - 글자 만들기는 `src/demoGuide.ts`(조사 이/가·이랑/랑·으로/로 — ㄹ받침은 `로`), 검사 `scripts/check-demo-guide.mjs`.
