...
- **먹은 기록:** 스냅숏과 함께 `food_logs.nutrition_incomplete`(JSON, NULL 가능, 마이그레이션 `h3i3n3c3m3p3`, down `h2c2o2o2k2l2`)에 레시피·칸 계산의 `incomplete`를 남긴다 — 무엇·양을 다시 계산할 때마다 새로 쓰고, 영양이 사라지면 NULL, 출처 없이 비율만 고칠 때는 그대로. 응답 `incomplete`는 그 값에 스냅숏에서 None인 영양소(음식 행에 없는 값·AI 추정 kcal만 있는 칸)를 `[기록 이름]`으로 더한 것(kcal이 없는 기록은 `{}`). 마이그레이션 전 기록은 어떤 값이 빠졌는지 알 수 없어 NULL로 둔다(다시 계산하면 스냅숏 값이 바뀌므로 채우지 않는다, 결정 2).
- **화면(`src/nutrition/day.ts`):** `addIncomplete`(이름을 한 번씩 모음) · `atLeast(incomplete, 영양소)`(값 뒤 ` 이상`) · `incompleteNotes(incomplete, 보이는 영양소)`(빠진 이름 목록이 같은 영양소끼리 한 줄: `된장은 나트륨 값이 없어 빼고 계산했어요` · `된장은 당류·나트륨 값이 …` · 둘 이상이면 `된장 외 1개는 …`, 조사는 `withJosa`) · `dailyValue`·`sugarDay`·`sodiumDay`의 세 번째 인자 `incomplete` — 빠진 값이면 기준치를 넘은 게 확실할 때만 돌려주고(`1일 기준치의 86% 이상`, `총 에너지의 13% 이상 · WHO 10% 미만`, 나트륨 초과는 `1일 기준치 넘었어요` 그대로), 아니면 `null`이라 비율 글자·막대를 그리지 않는다. kcal 표시는 그대로다.
- **레시피 상세 영양 카드·하루 영양 시트:** 빠진 영양소는 `탄수화물 30g 이상`·`나트륨 7mg 이상`, 탄단지 중 하나라도 빠지면 비율 막대(`.nt-split`)를 숨기고, 당류·나트륨 기준치 글자·막대는 위 규칙대로. 안내는 `.nt-note`(13px, `--text-2`)로 카드는 당류·나트륨 아래, 하루 시트는 당류·나트륨 상자 아래. 하루 시트는 `source calc` 칸의 이름만 모은다(AI 추정 칸은 원래대로 `kcal 일부는 AI 추정치예요`).
- **먹은 기록 날짜 상세(24절):** `dayTotals.incomplete`에 기록마다의 `incomplete`를 모아 설명을 `당류 24g 이상 · 나트륨 1,647mg 이상`으로, 합이 0이 아니어서 설명에 보인 당류·나트륨만 `dayLeftOutNotes`로 막대·경고 아래 안내한다. 나트륨 초과 경고(`.fl-warn`)는 빠진 값이 있어도 넘은 것이 확실해 그대로. 먹은 것 추가 시트의 레시피 미리보기도 `나트륨 500mg 이상`.
