...
- `src/nutrition/day.ts`(순수 로직): `daySum`(그날 채운 칸의 1인분 값 합, 탄단지·당류·나트륨은 `source calc` 칸만 더한다, 결정 14·15) · `dayHeadText`(하루 머리 `약 1,190 / 1,294kcal`) · `meterPercent` · `macroSplit`(탄단지 4·4·9kcal 비율) · `dailyValue`(레시피 1인분 1일 기준치 대비, warn 33% 초과) · `sugarDay`(총 에너지 대비 WHO 10%, warn 10% 이상) · `sodiumDay`(2,000mg 대비, warn 초과) · `goalFor`(몸 정보 목표 → 식단 `goal_kcal` → null, 결정 4) · `fillTargets`(보이는 주 날짜의 pending 레시피만, 중복 없이 31개까지, 결정 13). `slotKcalText`는 Task 5 `SlotNutrition`을 `"310kcal"`/`"약 480kcal"`/`""`로.
...
- **하루 영양 시트(`DayNutritionSheet.tsx`, `{date, slots, goal, onClose}`):** 그날 채운 칸을 `daySum`으로 더해 kcal/목표 막대, `source calc`인 칸이 하나라도 있으면 탄단지 비율 막대·범례(`.nt-split`·`.nt-legend`)와 당류·나트륨 상자(`sugarDay`·`sodiumDay`, 초과 시 주황), AI 추정 칸이 섞이면(`hasAi`) 상자 아래(또는 계산 칸이 없으면 단독으로) `kcal 일부는 AI 추정치예요`(결정 16). 칸마다 `아침 · 토스트  310kcal`(kcal 없으면 `—`), 끝에 참고용 문구.
...
- `dish_items(q)`: `search_items`와 같은 정규화 키(`query_key`)·이스케이프한 `ilike`로 캐시에서 **음식**(`group_name == DISH_GROUP`, `source != "ai"`) 행만 200개까지 → `(key가 name_parts(이름)에 없음, 이름 길이, 이름)` 순 정렬 → 앞 `SEARCH_LIMIT`(20)개. `search_items`와 달리 이미 음식 행만 걸러서 `GROUP_ORDER` 정렬은 없고, `kcal`도 반올림하지 않는다(양으로 곱하는 건 화면 몫).
