...
6. **요리했어요 폼**: 냉장고와 매칭된 재료 목록 + 사용량(기본 전량), 날짜(기본 오늘), 별점, 메모, 사진 → 저장. (구현 29절)
...
- **화면: 요리했어요 시트·되돌리기 알림·레시피 상세 요리 표시(29절 결정 2·3·16·26, Task 8, 시안 1·2):** 입구는 내 레시피 상세 아래 `cta-bar`의 `요리했어요`(공공 레시피 상세에는 없음, 결정 2 — 식단 칸은 Task 9). 시트(`CookSheet`)는 초안 GET을 받은 뒤 그리고 닫으면 초안 캐시를 버린다(다시 열 때 옛 재고·추정 전 값으로 채우지 않게). 제목 `{레시피} 요리했어요`, 설명 `쓴 재료는 재고에서 빼요`. 인분 −/+ 1~20(결정 3, 기본 레시피 인분·식단 칸이면 칸 인분)을 바꾸면 직접 고치지 않은 줄의 쓴 양만 `base_amount × 인분 ÷ 레시피 인분`(소수 셋째, 모르면 1)으로 다시 채운다. 재고에 있는 줄마다 체크(재고에 있고 양념이 아니면 켜짐, 이름을 눌러도 체크)·재고 단위 쓴 양 칸·`재고 1kg`(양념은 `양념 · 기본으로 안 빼요`)·쓴 양 ≥ 재고면 `마저 써요`, 재고에 없는 재료는 목록 아래 `재고에 없는 재료: 돼지고기 · …` 한 줄. 언제 칩 `오늘 · 어제 · 날짜 고르기`(오늘까지만), 사 먹으면 얼마(1인분) 칸은 입력하는 대로 쉼표를 넣고 숫자로 못 읽으면 쓴 글자 그대로 두고 `0~1,000,000원 사이로 입력해주세요`, 값이 없고 AI를 쓸 수 있으면 체험 계정이 아닐 때만 레시피마다 한 번 자동 추정(`추정하는 중…` → `추정 · 고칠 수 있어요`, 실패는 조용히), 체험 계정은 빈 칸 옆 `추정해줘요`를 누를 때만(실패하면 서버 문구, 개정 1 P16). 별점(같은 점수를 다시 누르면 지움, `StarPicker`), 사진 한 장(`찍기`·`앨범`, 사진 저장소가 꺼져 있으면 없음 — 줄이지 못한 원본은 사진 정보가 남을 수 있어 올리지 않는다), `메모 (선택)` 500자, `먹은 기록에도 남기기` 스위치(기본 켜짐, 설명 `9월 15일 저녁 · 집밥` — 끼니는 결정 16 `cookMeal`, 이미 먹은 식단 칸이면 꺼짐·막힘 `이미 먹은 기록이 있어요`). `저장하고 재고에서 빼기`는 틀린 칸이 있으면 막고 첫 틀린 칸으로 포커스, 저장하면 레시피·먹은 기록·식단·요리 일기 캐시를 지우고 시트를 닫는다. 저장 뒤 앱 전체에 하나인 아래 알림(`UndoToast`, `role=status`): `재고에서 김치·두부·대파를 뺐어요`(4개부터 `외 N개`, 뺀 재료가 없으면 `요리 일기에 남겼어요`) + 계산했으면 `약 10,800원 아꼈어요`(100원 단위, 음수는 `더 들었어요`) + `되돌리기`. 10초 뒤 사라지고 손가락·마우스·포커스가 알림 위에 있으면 멈췄다가 떠나면 남은 시간부터(결정 7, WCAG 2.2.1), 새 저장 알림이 오면 바꾸고 로그아웃하면 지운다. 되돌리면 `재고를 되돌렸어요`(건너뛴 재료가 있으면 `· 대파는 그사이 바뀌어 그대로 뒀어요`) 3초, 실패하면 서버 문구 5초. 알림은 탭 막대 앞(요리했어요 다음 Tab이 되돌리기)이고, 떠 있는 동안 레시피 상세 `요리했어요` 버튼을 알림 위로 올린다. 알림 초록 글자는 `--accent`를 알림 글자색 쪽으로 섞어 라이트·다크 모두 4.5:1을 넘긴다. 되돌린 뒤에는 그때 떠 있는 레시피 상세만 다시 받는다(떠난 화면은 건드리지 않고, 떠났다 돌아온 같은 레시피 화면은 새로 — 옛 `요리 1번`이 남지 않게). 레시피 상세 머리 아래 `요리 N번` 알약 + `마지막 9월 15일 · ★★★★☆`(별점 없으면 날짜만, 스크린리더는 `별점 4점`). 표시 수식은 `src/cooklog/cook.ts`, 검사 `scripts/check-cooklog.mjs`.
