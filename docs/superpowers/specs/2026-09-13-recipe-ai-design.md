...
- **쿠팡 파트너스 제휴 링크 (2026-09-15, 구현):** 파트너스 딥링크 API는 비밀키 HMAC 서명이 필요해 서버가 만든다(`backend/app/coupang.py`).
  ...
  - 열기: `AFFILIATE_FORMATS.coupang`이 일반 검색 주소를 `/api/shop-links/coupang/go?url=<일반 주소>`로 감싼다. 칩은 지금처럼 새 창 `<a>`라 누른 순간 열려 팝업 차단이 없고, 서버가 302로 보낸다(`Cache-Control: no-store`, 로그인 필요). 서버는 `https://www.coupang.com/np/search`의 `q`(1~50자)·`sorter`(표에 있는 값)만 받아 주소를 다시 만들고 나머지는 400.
  - 딥링크: `POST https://api-gateway.coupang.com/v2/providers/affiliate_open_api/apis/openapi/v1/deeplink` `{"coupangUrls": [주소]}` → `rCode "0"`이면 `data[0].shortenUrl`(https·`*.coupang.com`만). 서명 `CEA algorithm=HmacSHA256, access-key=…, signed-date=yyMMdd'T'HHmmss'Z'(UTC), signature=hex(HMAC_SHA256(secret, signed-date+method+path+query))`. 고정 호스트 요청(`outbound.fetch_fixed`, 리다이렉트·프록시 없음) 3초.
  ...
  - 실패: 키 없음·한도 초과·API 오류·3초 초과·이상한 응답이면 그 순간 일반 검색 주소로 302(로그는 예외 이름만). 실패는 기억하지 않는다.
