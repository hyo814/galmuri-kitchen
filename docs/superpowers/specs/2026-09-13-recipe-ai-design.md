...
- **영상 설명·캡션 미리 거르기 (2026-09-16, 사용자 승인):** 유튜브 설명(제목은 보지 않는다 — `계란 2개로…` 같은 제목은 레시피가 아니다)이나 인스타그램 캡션에 블로그와 같은 재료·양 표시(`RECIPE_SIGNAL`)가 하나도 없으면 AI를 부르지 않고 422 `need_text`(`link` 기록·토큰 없음, 외부 요청 `link_fetch` 기록은 그대로). 문구는 유튜브 `영상 설명에 레시피가 없어요. 쇼츠처럼 레시피가 영상에만 있으면, 보면서 재료와 만드는 법을 적어 붙여 넣어주세요.`, 인스타그램 `게시물 설명에서 레시피를 찾지 못했어요. 설명을 길게 눌러 복사해 붙여 넣거나, 레시피가 영상에만 있으면 보면서 재료와 만드는 법을 적어 붙여 넣어주세요.`(미리보기 캡션은 잘릴 수 있다). 설명이 비었거나 10자 미만이어도 이 문구다. 계기: 설명에 재료가 없는 쇼츠 링크(`m.youtube.com/shorts/…`)가 AI를 불러 하루 횟수만 쓰고 `need_text`로 끝났다(자막은 가져오지 않는다). 블로그는 본문 사진을 함께 읽을 수 있어 그대로, 붙여 넣은 글도 거르지 않는다.
