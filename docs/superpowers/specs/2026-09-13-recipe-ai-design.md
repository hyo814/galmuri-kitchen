...
- **요리 일기 테이블(29절 결정 1·19, Task 4, 마이그레이션 `h2c2o2o2k2l2`):** `cook_logs`(4절 칸 + servings·사 먹으면 얼마·출처·재료비·아낀 돈·제외 수·`food_log_id`·`photo_size`·`updated_at`, 인덱스 `user_id, cooked_on`·`recipe_id`·`food_log_id`)와 `cook_log_items`(쓴 재료 한 줄 — 계산표·되돌리기 스냅숏, `location_id`는 FK가 아닌 스냅숏). 돈 칸은 저장할 때 계산한 값이라 레시피·재고를 고쳐도 지난 일기는 그대로다.
- **요리 일기 저장 `POST /api/cook-logs`(29절 결정 6·9·11·16·19, `app/cooklog.py`):** multipart `data`(JSON 문자열) + 선택 `image` → 201 `{log, deducted_names}`(뺀 재료 이름, 요청 순서). `data` = `{recipe_id, servings(1~20), cooked_on, rating?, memo?(500자), eat_out_price?(0~1,000,000), usages: [{ingredient_id, amount(0 < x ≤ 100,000)}] 0~50개, food_log?(기본 true), meal?, meal_slot_id?}`. 순서: 입력 검사(내 레시피 404, 식단 칸은 같은 레시피만, 먹은 기록을 남기는데 칸 날짜와 요리 날짜가 다르거나 칸이 없으면 `끼니를 골라주세요.`) → 사진 검사(저장소 off 503, 3MB·서명) → **사용자 잠금**(PostgreSQL `pg_advisory_xact_lock(crc32("cook_logs"), user_id)`) → 일기 5,000개·사진 합계 200MB(체험 20MB, 일기 사진만) 확인 → 차감 전 초안(`draft_rows`)으로 양념 판정 → 쓴 재료 행 `FOR UPDATE`(id 순, 잠근 뒤 값으로 다시 읽음) — 없거나 남의 id면 400 `재고가 방금 바뀌었어요. 다시 불러와주세요.` → 줄마다 `used = min(쓴 양, 재고)`, `남은 양 = round(재고 − 쓴 양, 3)`이 0.001 미만이면 행 삭제 + 같은 커밋에 `ingredient_removals(reason = eaten)`, 스냅숏 줄(양념은 `excluded seasoning`·재료비 없음 — 켜서 빼도 재료비에 넣지 않음, 가격·구입 수량 없으면 `no_price`) → 초안에서 재고에 없던 양념 아닌 재료는 이름·레시피 양만(`no_price`) → `summarize`로 재료비·아낀 돈·제외 수 → 먹은 기록(결정 16: `food_logs.build_log` 하나로 만들고 `source cook_log`·`place home`·1인분·영양 스냅숏, 칸 날짜와 요리 날짜가 같을 때만 칸에 연결, 이미 먹은 칸이면 만들지 않음, 하루 20개에 걸리면 저장 전체 400) → 사진을 `cooklog/<user_id>/<uuid4 hex>.<ext>`로 올림(저장소 오류 503 — 커밋 전이라 모두 되돌려짐) → 커밋. 커밋이 실패하면 올린 파일을 지우고, `IntegrityError`(같은 칸 먹은 기록 경합 등)는 로그에 타입 이름만 남기고 400 `재고가 방금 바뀌었어요. 다시 불러와주세요.`. 사 먹으면 얼마(결정 11): 비워 보내면 일기만 비우고 레시피는 그대로, 레시피 값과 같으면 레시피 출처를 스냅숏, 다르면 일기·레시피를 `user`로(레시피 `updated_at`은 그대로 — 목록 순서). 응답 `log` = `{id, recipe_id, title, cooked_on, servings, rating, memo, photo_url, eat_out_price, eat_out_source, ingredient_cost, saved, excluded_count, created_at, items: [{name, amount_text, used, unit, removed, price, price_quantity, cost, excluded}], food_log_id, undo_until}`. `ponytail:` 사진을 올리는 동안(최대 수 초) 사용자 잠금·재료 행 잠금을 잡고 있다 — 같은 사용자 요청만 기다린다. 먹은 기록 메모 검사는 `validation.memo(value, max_len)` 하나를 함께 쓴다(개정 1 D14).
- **되돌리기 `POST /api/cook-logs/<id>/undo`(29절 결정 7·8):** 사용자 잠금 → 내 일기(아니면 404) → 서버 시각이 `created_at + 120초`를 넘으면 400 `되돌릴 수 있는 시간이 지났어요. 재고는 직접 고쳐주세요.`(SQLite의 시간대 없는 값은 UTC로 보고 비교). 줄마다(id 순): 차감으로 지운 줄은 스냅숏(이름·수량·단위·위치·구입일·유통기한·가격·구입 수량)으로 새 행을 만들고(위치가 지워졌거나 남의 것이면 기본 위치, 재료 2000개 상한은 보지 않음) `다 먹었어요` 기록을 지운다, 남은 재료는 `FOR UPDATE`로 읽어 뺀 양만큼 더한다(그사이 고친 수량 위에), 사용자가 지웠거나 단위를 바꾼 재료는 `skipped`, 재고에 없던 줄은 목록에 넣지 않는다. 함께 만든 먹은 기록(그사이 붙인 사진 포함)·일기를 지우고 커밋 뒤 일기 사진·먹은 기록 사진 파일을 지운다 → 200 `{restored: [이름], skipped: [이름]}`. 일기를 지우므로 두 번째 요청은 404(잠금을 먼저 잡아 동시에 두 번 보내도 한 번만 더함). 사 먹으면 얼마로 바꾼 레시피 값은 되돌리지 않는다.
