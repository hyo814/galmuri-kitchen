...
- **체험 계정 요리 일기 예시(29절 결정 30, Task 7, `app/demo.py`):** 예시 재료 중 두부·대파·김치·돼지고기 앞다리살에 `INGREDIENT_PRICES`로 가격·구입 수량(만들 때 수량 그대로)을 미리 넣는다. 체험 레시피 둘(김치찌개·된장찌개) 모두 `EAT_OUT_PRICES`로 사 먹으면 얼마를 미리 넣는다 — 체험은 요리했어요 시트를 열어도 자동으로 추정하지 않으므로(개정 1 P16) 값 없는 레시피가 남지 않게 한다(김치찌개는 `sample` = AI 예시 모드와 같은 9,000원, 된장찌개는 `user` = 직접 넣은 모양). 요리 일기 3개(`COOK_LOGS`, 오늘로부터 2·5·35일 전, `created_at`은 요리한 날 서울 19시 — 기본값(만든 시각)이면 체험하기 직후 2분 동안 되돌리기가 예시 일기를 지웠다, 2026-09-16)를 `cooklog.item_cost`·`cooklog.summarize`·`cooklog.is_seasoning`으로 계산해 재료비·아낀 돈·가격 제외 수까지 채우고, 버린 재료 2개(`DISCARDED`, `ingredient_removals(reason="discarded")`)도 만든다. 재고 수량은 요리 일기 때문에 줄지 않는다(예시 재고는 그대로 둔다).
