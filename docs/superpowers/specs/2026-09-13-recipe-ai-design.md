...
- **AI 일일 한도**: 요청 전 오늘(서버 기준 Asia/Seoul) 해당 사용자의 `ai_calls` 수를 kind 그룹(scan: fridge+receipt+order+memo / recipe: recipe+link+meal (2026-09-14, 시안 승인; meal은 20절 AI 식단 초안))별로 센다. 서울 하루를 UTC 구간으로 바꿔 created_at으로 센다.
...
- **화면 (2026-09-14, 시안 승인 — `docs/design/meals-4b/` 15장):**
  - 식단 탭: 제목 아래 식단 고르기 버튼(`9월 셋째 주 ⌄`)과 주/월 전환, 오른쪽 `AI 초안`·`⋯`(이번 주 복사, 식단 이름·기간 고치기, 맨 아래 `식단 지우기`). 주 보기는 하루 카드 한 장에 아침·점심·저녁·간식 줄, 오늘은 초록 테두리, 빈 칸은 `+`. 월 보기는 월요일 시작, 날짜마다 끼니 점 4개, 누르면 그 주로. 하단 `장보기 목록 만들기`.
