...
- **화면 (2026-09-14, 시안 승인 — `docs/design/meals-4b/` 15장):**
  ...
  - 칸 상세 시트: 맨 위 `먹었어요`(24절), 인분 −/+ 는 바로 저장, `레시피 보기`, `다른 걸로 바꾸기`, 맨 아래 `칸 비우기`(배경+테두리).
...
| 묶음 | 항목 | 만드는 시점 |
|---|---|---|
| **기록** | 먹은 기록(24절 달력, 4b-3 완료) · 요리 기록(5단계) · 집밥 리포트 | 4b · 5 · 집밥 리포트는 5단계 뒤 |
...
- **사 먹으면 얼마(A):** ① `recipes.eat_out_price`(사용자 입력) → ② 없으면 AI 추정 한 번(레시피 이름·인분, 결과를 레시피에 저장해 다시 묻지 않음, AI 일일 한도 recipe 그룹, 화면 `추정`). 참가격 외식비는 API 이용 조건이 확인되면 ②보다 앞에 붙인다. 폼에서 값을 바로 고칠 수 있고 고치면 레시피에 저장한다.
- **저장 후(B):** 레시피 상세로 돌아와 알림 `재고에서 김치·두부·대파를 뺐어요 · 약 N원 아꼈어요` + `되돌리기`(10초, 재고 수량·삭제된 재료 복구 + 일기·먹은 기록 삭제). 그 뒤 일기를 지우면 기록만 지우고 재고는 되돌리지 않는다(상세에 안내 문구).
