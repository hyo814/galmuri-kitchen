...
- **레시피 기준 인분 (D1, 3단계):** `recipes.servings`(int 1~20, 기본 2) 추가. AI 생성·링크 가져오기는 인분을 추정해 채우고 확인 화면에서 수정.
  ...
  인분을 바꿔 양이 달라진 큰술·작은술 재료는 양 아래 회색 줄에 22절 계산기와 같은 환산(`seasoning.ts` `spoonHint` → `scaleItem`)을 보여준다 (2026-09-16, 사용자 승인): 큰술은 `밥숟가락 약 4개`, 3작은술 이상은 `1큰술`. 계산기처럼 ½컵(100ml) 이상은 `¾컵`으로 보여준다. 1큰술이 안 되는 양·g·개·약간·범위·레시피 인분 그대로면 줄이 없다. 스크린리더는 양 다음에 쉼표로 이어 읽는다. `요리했어요` 시트는 레시피 양 글자를 보여주지 않아 붙이지 않았다.
