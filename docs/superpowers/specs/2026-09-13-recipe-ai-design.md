...
- **화면 캡처로 가져오기 (2026-09-16, 사용자 승인):** 유튜브·인스타그램 링크를 못 읽어(422 `need_text`) 글 붙여넣기로 바뀌면(영상 보기의 `레시피로 가져오기`도) 경고 상자 아래에 보조 버튼 `화면 캡처로 가져오기`와 안내 한 줄 `영상을 멈추고 재료와 만드는 법이 나온 화면을 캡처해 올려주세요 · 5장까지`를 두고, 누르면 경고를 지우고 아래 `사진으로 가져오기` 단계로 바뀐다(포커스는 시트 제목). 이렇게 온 사진 단계는 캡처한 화면이 앨범에 있어 `앨범에서 고르기`(주 버튼) → `카메라로 찍기` 순서이고(캡처 버튼 아이콘도 앨범과 같다), 그 아래 `취소`(테두리 버튼)는 글 단계로 돌아가 쓰던 글·경고·캡처 버튼을 다시 보여준다. 사진을 못 읽어도 이 모양은 그대로이고, 방법 고르기에서 고른 사진 단계는 예전 그대로다. 링크 구분은 화면에서 입력한 주소의 호스트로 한다 — 서버와 같이 앞의 `www.`·`m.` 하나를 뗀 `youtube.com`·`music.youtube.com`·`youtube-nocookie.com`·`youtu.be`·`instagram.com`. 블로그 링크·처음부터 글 붙여넣기의 실패, 글 단계에서 다시 정리하기를 누른 뒤의 경고에는 두지 않고, 글 붙여넣기 칸·포커스는 그대로다.
