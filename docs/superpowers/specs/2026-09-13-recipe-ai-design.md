...
- **흐름:** 고르기 → 사진 확인(시트 제목 `<이름> 예시`, 사진 전체를 자르지 않고, 설명, `이 사진을 AI가 지금 읽어요 · 오늘 N번 남음`, `다른 사진`·`이 사진 읽기`) → 찾는 중(사진 위를 훑는 줄 — 움직임 줄이기면 멈춤, 단계 목록 `사진 보냈어요`(끝남)·`글자와 품목을 읽는 중`(지금)·`식재료만 골라 보관 위치 정하기`, 가짜 타이머 없이 응답이 오면 넘어간다, `취소`는 사진 확인으로) → 확인. 확인·실패 화면의 왼쪽 버튼은 `다른 사진`(고르기로). 사진을 못 받으면 `예시 사진을 불러오지 못했어요. 다시 시도해주세요.`
...
- **A. 사진과 결과 비교 = ⑤-A 사진 띠:** 확인 화면 위에 150px 띠(사진 전체), 목록을 내려도 위에 붙어 있고 `사진 접기`/`사진 펼치기`(`aria-expanded`·`aria-controls`)로 접는다. 띠가 포커스한 칸을 가리지 않게 시트에 `scroll-padding-top`. 설명 `<이름> 1장을 N초 만에 읽었어요 · 넣을 재료만 체크해주세요`(N = 요청 직전부터 응답까지 화면에서 잰 초, 반올림·최소 1). 시안의 `휴지·세제는 뺐어요`는 결과가 뺀 것을 알려 주지 않아 쓰지 않는다. 가격은 재료 줄에 원래 보인다.
...
- **체험 AI를 다 썼을 때(⑥):** 응답이 `sample: true`면 시간 문구를 빼고 안내를 `오늘 체험용 AI를 다 써서, 이 사진을 미리 읽어 둔 결과를 보여줘요. 로그인하면 내 사진을 바로 읽어요.`로(링크 없음, 로그인 방법 이름은 적지 않는다). 사진 확인 화면도 `오늘 체험용 AI를 다 써서, 이 사진을 미리 읽어 둔 결과를 보여줘요`. 띠는 그대로 둔다(저장된 결과가 그 사진의 결과다). 내 사진의 일반 예시 결과 안내는 그대로.
...
- **요청·응답:** `GET /api/me`·체험 로그인 응답의 `scan_samples: string[]` = 미리 읽어 둔 결과가 있는 사진 id, 체험 계정이고 사진 인식이 off가 아닐 때만(그 밖은 `[]`, 인터넷 없이 연 옛 사용자 정보에는 없을 수 있어 화면은 `[]`로 본다). 화면은 사진을 `/samples/<id>.jpg`에서 받아(같은 출처, 그림이 아니면 실패 — 없는 파일은 서버가 index.html을 준다) 내 사진처럼 줄여 `POST /api/scan?kind=<사진 종류>`에 폼 칸 `sample=<id>`를 더해 보낸다. 모드 `sample`(체험 예산 소진·키 없는 개발)에서 체험 계정·아는 id·종류가 맞으면 저장된 결과(`sample: true`), 아니면 일반 예시 결과. 모드 `on`은 내 사진과 똑같이 실제 AI(사용자 한도·체험 전체 예산에 셈). 저장된 결과는 `backend/app/data/sample_scans.json`(`ai.sample_scans()`), id와 종류는 `ai.SAMPLE_PHOTOS`.
...
- **검사:** E2E `e2e/scan-sample.spec.ts` — 체험 로그인 → 사진으로 추가 → 예시 사진 2장 → 냉장고 → 사진 확인 → 저장된 결과 16개(간장)·띠 접기 → 재고에 넣기.
