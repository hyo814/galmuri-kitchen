...
- **화면 캡처로 가져오기 (2026-09-16, 사용자 승인):** 유튜브·인스타그램 링크를 못 읽어(422 `need_text`) 글 붙여넣기로 바뀌면 경고 상자 아래에 보조 버튼 `화면 캡처로 가져오기`와 안내 한 줄 `영상을 멈추고 재료와 만드는 법이 나온 화면을 캡처해 올려주세요 · 5장까지`를 두고, 누르면 경고를 지우고 아래 `사진으로 가져오기` 단계로 바뀐다(포커스는 시트 제목). 이렇게 온 사진 단계는 캡처한 화면이 앨범에 있어 `앨범에서 고르기`(주 버튼) → `카메라로 찍기` 순서이고(캡처 버튼 아이콘도 앨범과 같다), `사진은 AI가 정리해요` 줄 아래 가로 가득한 `취소`(테두리 버튼, 글 붙여넣기처럼 맨 아래)는 글 단계로 돌아가 쓰던 글·경고·캡처 버튼을 다시 보여준다. 사진을 못 읽어도(422) 경고와 함께 이 모양은 그대로이고, 방법 고르기에서 고른 사진 단계는 예전 그대로다. 영상 보기에서 `레시피로 가져오기`를 못 하면 같은 글 붙여넣기 시트(캡처 버튼 포함)가 열리고, 그 뒤로는 영상 보기의 가져오기 버튼 아래에도 캡처 버튼(회색 바탕 위라 테두리 버튼)·안내가 늘 보인다 — 누르면 링크를 다시 읽지 않고(`link_fetch` 없음) 사진 단계 시트를 바로 열어, 시트를 닫고 영상을 멈춰 캡처한 뒤 다시 올 수 있다(이 시트의 `취소`는 시트를 닫는다). 캡처로 가져온 레시피는 출처를 사진이 아니라 못 읽은 링크로 남긴다 — `source` youtube·instagram, `source_url`은 서버와 같은 표준 주소(`https://www.youtube.com/watch?v=ID`·`https://www.instagram.com/p/코드/`), 외부 요청이 없어 출처 카드 대신 확인 화면에 `유튜브 영상`(`인스타그램 게시물`)과 `원본` 링크만 보인다. 링크 구분은 입력한 주소로 한다 — 서버 `parse_link`와 같은 규칙(앞의 `www.`·`m.` 하나를 뗀 `youtube.com`·`music.youtube.com`·`youtube-nocookie.com`·`youtu.be`의 영상 ID 11자, `instagram.com`의 `p`·`reel`·`reels` 코드). 블로그 링크·처음부터 글 붙여넣기의 실패와 글 단계에서 다시 `정리하기`를 누른 뒤의 경고에는 두지 않고, 글 붙여넣기 칸·포커스는 그대로다.
