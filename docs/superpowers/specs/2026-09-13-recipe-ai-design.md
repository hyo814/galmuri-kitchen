...
- **요리했어요 폼(시트):** 레시피 상세와 식단 칸 상세(20절, `먹었어요` 옆)에서 연다. 식단 칸에서 열면 그 칸의 요리·인분·날짜로 채운다.
