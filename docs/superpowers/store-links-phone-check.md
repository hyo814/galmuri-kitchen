...
확인 전에는 운영 화면의 쇼핑몰 시트가 쇼핑몰마다 정렬 칩 없이 `검색 결과` 링크 하나만 보여줘요(확인한 쇼핑몰만 정렬 칩, 개발 서버에서는 후보를 모두 보여줘요).
...
3. `정렬 후보 없음` 줄(컬리 · 홈플러스 · 롯데마트)은 검색어만 확인하고, 쇼핑몰 화면에서 정렬을 직접 바꾼 뒤 **주소창 주소를 복사해 보내주세요.** 정렬을 바꿔도 주소가 안 바뀌면 `주소 안 바뀜`이라고 보내주세요.
4. **안 된 줄만 채팅으로 보내주세요**(예: `쿠팡 · 새 상품순 — 오래된 순으로 떠요`). 안 보낸 줄은 된 걸로 볼게요. 한 브라우저에서만 안 되면 어느 쪽인지 같이 적어주세요.
5. 받은 내용으로 된 칸만 남기고 안 되는 정렬 칩은 숨기고, 검색까지 안 되는 쇼핑몰은 화면에서 빼요.
