...
class AiCall(db.Model):
    ...
    kind = db.Column(db.String(20), nullable=False)  # fridge | receipt | order | memo | recipe | link | recipe_photo | meal | nutrition(영양 추정, 토큰 있음) | link_fetch·channel_add·video_refresh·shop_link(외부 요청 기록)·export(데이터 내보내기)·food_fetch(식품영양성분 DB 요청), 모두 모델·토큰 없음
