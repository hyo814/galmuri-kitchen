...
from app.models import ShoppingItem, User, db
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"source": "ai"}, "잘못된 요청이에요."),
        ({"source_label": "가" * 61}, "레시피 이름은 1~60자로 입력해주세요."),
...
def test_cap_300_excludes_stocked(client, login, app):
    ...
    now = datetime.now(timezone.utc)
    with app.app_context():
        db.session.add_all([ShoppingItem(user_id=user.id, name=f"항목{i}") for i in range(299)])
...
@pytest.mark.parametrize(
    ...
    [
        {"source": "recipe", "items": []},
        {"source": "recipe", "items": [{"name": "우유"}] * 51},
        {"source": "recipe", "items": "우유"},
        {"source": "ai", "items": [{"name": "우유"}]},
        [],
    ],
)
def test_bulk_rejects_bad_shape(client, login, body):
    login()
    assert client.post("/api/shopping/items/bulk", json=body).status_code == 400
...
def test_check_last_change_wins(client, login):
    ...
    assert done_at < future - timedelta(minutes=50)
