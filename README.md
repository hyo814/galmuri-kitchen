...
<div align="center">
  <video src="https://github.com/user-attachments/assets/015fae60-c9c9-4921-b54c-19000a118f7e" poster="https://raw.githubusercontent.com/hyo814/galmuri-kitchen/main/docs/submission/screenshots/34-demo-guide.png" controls muted playsinline></video>
</div>
...
- **링크·글 가져오기**: 유튜브(설명란)·블로그·인스타그램 링크나 붙여 넣은 글을 AI가 재료·만드는 법으로 정리합니다. 확인 화면에서 고친 뒤 저장합니다. 한 페이지에 요리가 여러 개(레시피 모음 글 등)면 목록으로 모두 찾아 최대 5개까지 골라서 가져옵니다. 링크를 못 읽으면 화면 캡처로 가져오기를 안내합니다. 유튜브·인스타그램 링크는 서버가 실제로 그 종류의 주소인지 다시 확인하고, 같은 링크를 다시 가져오면 `이미 저장한 레시피예요 · 레시피 보기`로 안내합니다.
- **사진으로 가져오기**: 요리책·캡처·손글씨 사진을 찍으면 AI가 레시피로 정리합니다.
- **영상**: 기본 요리 채널 14개가 들어 있고, 내 채널은 30개까지 추가합니다. 전체 영상 목록은 채널당 최신 12개까지 섞고, 채널 칩은 짧은 이름으로 보여 줍니다. 검색 칸에서 영상 제목·설명으로 찾고, 채널을 고르면 그 채널 안에서 YouTube로 바로 찾습니다. 고른 채널의 새 영상만 앱 안에서 보고 `레시피로 가져오기`를 누릅니다.
- **양념 비율**: 주재료 무게·인분·완성량 기준으로 늘리고 줄입니다. 결과는 `3½큰술`처럼 실제로 뜰 수 있는 분수로, 아래에 `밥숟가락 약 4개`를 함께 보여 줍니다.
- **레시피 상세**: 인분 −/+, 가진 재료 표시, `없는 재료 N개 장보기에 담기`. 재료 줄마다 재고에 남은 양을 `있어요 300g`으로 보여 주고, 요리 직후 다 쓴 재료는 `다 썼어요`로 바뀝니다. 인분을 바꾸면 큰술·작은술 재료 아래에 밥숟가락 환산 줄도 같이 바뀝니다. 로그인 화면을 거쳐도 보던 레시피 화면으로 돌아옵니다.
...
- **쇼핑몰 검색**: 쿠팡·네이버 쇼핑·컬리·이마트몰·롯데마트·G마켓 6곳 검색 결과로 이동합니다. 결제는 쇼핑몰에서 합니다. 6곳 모두 폰에서 확인한 `낮은 가격순` 정렬 칩을 보여 줍니다.
- **쇼핑몰에서 한꺼번에 찾기**: 살 것이 여러 개면 쇼핑몰을 한 번만 고르고 품목마다 찾기 링크를 차례로 엽니다. 몇 개 열어봤는지 보여 주고, 지난번 고른 쇼핑몰을 다음에 맨 앞으로 기억합니다.
...
- **영양 계산**: 내 몸 정보(성별·출생연도·키·몸무게·활동량·목표)로 하루 칼로리 목표를 정하고, 레시피 1인분 영양(탄수화물·단백질·지방·당류·나트륨)과 식단 하루 합계를 봅니다. 재료는 식품영양성분 DB에서 찾고, 못 찾은 재료만 AI가 추정해 `추정`으로 표시합니다. 일부 재료만 영양 값이 빠지면 합계를 `이상`과 안내로 보여 주고, 기준치 비율·막대는 값이 확실할 때만 그립니다.
- **요리 일기**: 더보기 탭에서 화면까지 동작합니다. 레시피 상세·식단 칸의 `요리했어요`로 쓴 재료를 재고에서 빼고(10초 알림에서 되돌리기), 별점·사진·메모와 아낀 돈(사 먹으면 얼마 × 인분 − 쓴 재료비)을 남깁니다. 레시피 없이 요리 이름과 재료비(선택)만 적어 직접 쓸 수도 있고, 나중에 고치기에서 이름도 바꿉니다. 먹은 기록 달력에 `요`로 표시됩니다.
- **집밥 리포트**: 한 달 동안 아낀 돈·요리 횟수·기록한 날·집밥 비율·버린 재료·지난달 대비·많이 아낀 요리를 봅니다.
...
| AI 비용 | 사용자별 하루 사진 인식 20번·AI 레시피 20번(가져오기·식단 초안·사 먹으면 얼마 추정 포함, 서울 날짜), 60초 안 3번 연속 제한. 영양 추정은 따로 하루 20번(체험 계정 2번, 체험 전체 하루 30번). 로그인 사용자 전체도 24시간 100번(`USER_AI_GLOBAL_DAILY`, 그중 영양 추정 하루 40번)에서 멈춰 체험 계정 몫을 지킵니다. 찾지 못했거나 실패한 호출은 사용자마다 하루 3번까지 하루 한도에서 빼고(`<kind>_miss`, 연속 제한·전체 예산에는 셉니다) 그 뒤로는 셉니다. 설명에 재료가 없는 유튜브·인스타그램 링크는 AI를 부르지 않습니다. 호출마다 모델·토큰을 `ai_calls`에 남겨 원가를 계산합니다. |
| 체험 계정 AI | 계정당 하루 5번. 체험 계정 전체가 24시간에 80번(`DEMO_AI_GLOBAL_DAILY`, 배포 설정값)을 넘으면 AI를 부르지 않고 예시 결과를 보여 줍니다. 영상은 서버가 받아 둔 기본 채널 영상을 읽기만 해서 보기·검색은 유튜브 할당량을 쓰지 않습니다(`레시피로 가져오기`만 위 AI 한도 안에서 영상 1개당 1 unit). |
...
2. **로그인 없이 체험하기**를 누릅니다. 예시 재고 10개(두부 D-1, 대파 D-2 포함), 필수품 73개(모든 계정에 들어가는 기본 72개 + 간장 — 이 중 간장만 재고에 없어 첫 화면 배너에 뜹니다), 레시피 2개, 양념 비율 1개, 장보기 목록·메모, 한 주 식단, 먹은 기록 3개가 든 계정이 열립니다.
...
| 순서 | 내용 |
|---|---|
| 완료 | Render 배포, 4b-2 영양 계산, 4b-3 먹은 기록, 5 요리 일기·집밥 리포트, 레시피 상세 재고 남은 양·다 썼어요 표시, 쇼핑몰에서 한꺼번에 찾기, 여러 요리 가져오기, 요리 일기 직접 쓰기, 식약처 레시피 전체 동기화, 쇼핑몰 6곳 정렬 링크 폰 확인 |
| 운영 | 투표 기간 운영(AI 한도·사용량 알림·로그 확인) |
...
| 방식 | 상태 | 내용 |
|---|---|---|
| 쿠팡 파트너스 제휴 링크 | 구현(키는 보류) | 장보기에서 쿠팡으로 갈 때 제휴 링크로 보내고 `광고` 표시를 붙이도록 코드는 준비돼 있습니다. 키 발급은 누적 판매 15만 원을 넘겨야 최종 승인이 나 심사·투표 기간 안에는 받을 수 없어, 지금은 일반 검색 링크입니다. 체험 계정·로그인 전도 늘 일반 링크입니다. |
