...
- **링크·글 가져오기**: 유튜브(설명란)·블로그·인스타그램 링크나 붙여 넣은 글을 AI가 재료·만드는 법으로 정리합니다. 확인 화면에서 고친 뒤 저장합니다. 한 페이지에 요리가 여러 개(레시피 모음 글 등)면 목록으로 모두 찾아 최대 5개까지 골라서 가져옵니다. 링크를 못 읽으면 화면 캡처로 가져오기를 안내합니다.
...
- **레시피 상세**: 인분 −/+, 가진 재료 표시, `없는 재료 N개 장보기에 담기`. 재료 줄마다 재고에 남은 양을 `있어요 300g`으로 보여 주고, 요리 직후 다 쓴 재료는 `다 썼어요`로 바뀝니다. 인분을 바꾸면 큰술·작은술 재료 아래에 밥숟가락 환산 줄도 같이 바뀝니다.
...
```
backend/.venv/bin/pytest -q -W error::DeprecationWarning   # 백엔드 1,605개
cd frontend && npm run check && npm run build              # 순수 로직 검사 스크립트 11개 + 타입 검사·빌드(빌드 전에 styles.css 괄호 짝 검사)
cd frontend && npm run e2e                                 # 화면 단위 테스트(Playwright) 74개 — 처음 한 번 npx playwright install chromium
```
...
| 항목 | 수치 |
|---|---|
| 기간 | 2026-09-13 10:51 첫 커밋 → 2026-09-17 20:59 `901fa47`(약 106시간) |
| 커밋 | 722개(병합 248) |
...
| 테스트 | 백엔드 1,605개(SQLite·PostgreSQL 통과), 프론트 검사 스크립트 11개, E2E(Playwright) 74개 |
