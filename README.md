...
- **요리 일기**: 더보기 탭에서 화면까지 동작합니다. 레시피 상세·식단 칸의 `요리했어요`로 쓴 재료를 재고에서 빼고(10초 알림에서 되돌리기), 별점·사진·메모와 아낀 돈(사 먹으면 얼마 × 인분 − 쓴 재료비)을 남깁니다. 먹은 기록 달력에 `요`로 표시됩니다.
...
```
backend/.venv/bin/pytest -q -W error::DeprecationWarning   # 백엔드 1,478개
cd frontend && npm run check && npm run build              # 순수 로직 검사 스크립트 10개 + 타입 검사·빌드(빌드 전에 styles.css 괄호 짝 검사)
...
| 항목 | 수치 |
|---|---|
| 기간 | 2026-09-13 10:51 첫 커밋 → 2026-09-16 03:16 `662b928`(약 64시간) |
| 커밋 | 575개(병합 210, fix 138, feat 103, docs 66) |
...
| 테스트 | 백엔드 1,478개(SQLite·PostgreSQL 통과, `662b928`), 프론트 검사 스크립트 10개 |
...
| 순서 | 내용 |
|---|---|
| 완료 | Render 배포, 4b-2 영양 계산, 4b-3 먹은 기록, 5 요리 일기·집밥 리포트 |
| 운영 | 쇼핑몰 정렬 링크 폰 확인, 레시피 상세에 남은 재료 양·다 썼어요 표시, 식약처 레시피 전체 동기화, 회원 탈퇴 버튼 |
